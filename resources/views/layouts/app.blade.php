...
<head>
    ...
    <style>
        ...
        aside { overflow-y: auto; scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent; }
        aside::-webkit-scrollbar { width: 4px; }
        aside::-webkit-scrollbar-track { background: transparent; }
        aside::-webkit-scrollbar-thumb { background-color: #cbd5e1; border-radius: 2px; }
        aside::-webkit-scrollbar-thumb:hover { background-color: #94a3b8; }
        ...
        .dropdown-exit-active { opacity: 0; transform: scale(0.95) translateY(-10px); transition: all 0.15s ease-in; }
        main { scroll-behavior: smooth; }
        body { overflow-y: scroll; }
        .nav-active { position: relative; }
        .nav-active::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 3px; height: 20px; background-color: #2563eb; border-radius: 0 2px 2px 0; }
        .admin-section { background: linear-gradient(135deg, #f0f4ff 0%, #e0edff 100%); border: 1px solid #c3d9ff; }
        .menu-group { position: relative; }
        .menu-group:not(:last-child)::after { content: ''; position: absolute; bottom: -8px; left: 12px; right: 12px; height: 1px; background: linear-gradient(90deg, transparent 0%, #e5e7eb 20%, #e5e7eb 80%, transparent 100%); }
        .alert { transition: opacity 0.3s ease, transform 0.3s ease; }
        .notification-item:hover, .document-item:hover { transform: translateX(2px); transition: transform 0.2s ease; }
        ...
        #successAlert { animation: slideIn 0.3s ease-out; transition: all 0.3s ease; }
        @keyframes slideIn { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }
    </style>
</head>
<body class="bg-gray-50">
    @if(Auth::check())
        {{-- (Seluruh bagian HTML <header>, <aside>, dan <main> Anda tetap sama) --}}
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="relative">
                            <button id="notificationBtn"
                                ...
                                <span id="notificationBadge" class="absolute -top-1 -right-1 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-red-600 rounded-full hidden">0</span>
                            </button>
                            ...
                            <div id="notificationDropdown"
                                ...
                                <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                                    ...
                                    <button id="markAllReadBtn" class="text-xs text-blue-600 hover:text-blue-800 font-medium">Tandai Semua Dibaca</button>
                                </div>
                                ...
                                <div class="px-4 py-3 border-t border-gray-200">
                                    <a href="{{ route('konsultasi.index') }}" class="block text-center text-sm text-blue-600 hover:text-blue-800 font-medium">Lihat Semua Advokasi & Aspirasi</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            <button id="userMenuButton" class="flex items-center space-x-2 text-gray-700 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg px-2 py-1">
                                @if(Auth::user()->profile_picture)
                                    <img src="{{ asset('storage/profile-pictures/' . Auth::user()->profile_picture) }}" alt="Profile Picture" class="w-7 h-7 rounded-full object-cover border border-gray-200">
                                @else
                                ...
                                <svg class="w-4 h-4 transition-transform duration-200" id="userMenuChevron" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>
                            <div id="userDropdown" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
                                ...
                                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50">
                                    <div class="flex items-center space-x-3">
                                        @if(Auth::user()->profile_picture)
                                            <img src="{{ asset('storage/profile-pictures/' . Auth::user()->profile_picture) }}" alt="Profile Picture" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                                        @else
                                ...
                                <div class="py-1">
                                    <a href="{{ route('profile.index') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                        <svg class="w-4 h-4 mr-3 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z" clip-rule="evenodd"></path></svg>
                                        <span>Profile</span>
                                    </a>
                                </div>
                                <div class="py-1">
            ...
            <nav class="h-full overflow-y-auto py-6">
                ...
                <div class="px-3 space-y-2">
                    <div class="menu-group pb-4">
                        <div class="px-3 mb-3"><p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Menu Utama</p></div>
                        <a href="{{ route('home') }}" class="flex items-center px-3 py-2.5 {{ request()->routeIs('home') ? 'text-blue-600 bg-blue-50 nav-active font-medium' : 'text-gray-600 hover:bg-gray-50' }} rounded-lg text-sm transition-all duration-200">
                        ...
                        <a href="{{ route('dashboard') }}" class="flex items-center px-3 py-2.5 {{ request()->routeIs('dashboard') ? 'text-blue-600 bg-blue-50 nav-active font-medium' : 'text-gray-600 hover:bg-gray-50' }} rounded-lg text-sm transition-all duration-200">
                    ...
                    <div class="menu-group">
                        <div class="px-3 mb-3"><p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Layanan</p></div>
                        <a href="{{ route('konsultasi.index') }}" class="flex items-center px-3 py-2.5 {{ request()->routeIs('konsultasi.*') ? 'text-blue-600 bg-blue-50 nav-active font-medium' : 'text-gray-600 hover:bg-gray-50' }} rounded-lg text-sm transition-all duration-200">
                    ...
                    <div class="menu-group">
                        <div class="px-3 mb-3"><p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Administrasi</p></div>
                        <div class="admin-section rounded-lg p-3 mb-3">
                            <a href="{{ route('data-anggota.index') }}" class="flex items-center px-3 py-2.5 {{ request()->routeIs('data-anggota.*') ? 'text-blue-600 bg-white shadow-sm nav-active font-medium' : 'text-gray-600 hover:bg-white/50' }} rounded-lg text-sm transition-all duration-200">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                                <span>Data Anggota</span>
                            </a>
                            <a href="{{ route('banpers.index') }}" class="flex items-center px-3 py-2.5 {{ request()->routeIs('banpers.*') ? 'text-blue-600 bg-white shadow-sm nav-active font-medium' : 'text-gray-600 hover:bg-white/50' }} rounded-lg text-sm transition-all duration-200">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Logika Dropdown Pengguna ---
    if (userMenuButton && userDropdown) {
        userMenuButton.addEventListener('click', function(e) {
            e.stopPropagation();
            // {{-- PERUBAHAN DI SINI: Tutup dropdown lain sebelum membuka yang ini --}}
    ...
    if (documentBtn && documentDropdown) {
        documentBtn.addEventListener('click', function(e) {
            ...
            if (userMenuChevron) userMenuChevron.style.transform = 'rotate(0deg)'; // Reset chevron
    ...
    // {{-- PERUBAHAN DI SINI: Satu event listener untuk menutup semua dropdown saat klik di luar --}}
    document.addEventListener('click', function(e) {
        ...
        if (documentDropdown && !documentBtn.contains(e.target) && !documentDropdown.contains(e.target)) {
            documentDropdown.classList.add('hidden');
        }
    ...
    // --- Fungsi Fetch Dokumen --- (Tidak ada perubahan di fungsi ini)
    ...
    // --- Fungsi Fetch Notifikasi --- (Tidak ada perubahan di fungsi-fungsi ini)
    if (markAllReadBtn) {
    ...
    (async () => {
        try {
            ...
            console.error('Could not fetch initial unread count.');
        }
    })();
