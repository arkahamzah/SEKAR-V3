        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">
                    @if(auth()->user()?->pengurus?->role && in_array(auth()->user()->pengurus->role->NAME, ['ADM', 'ADMIN_DPP', 'ADMIN_DPW', 'ADMIN_DPD']))
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ $konsultasi->total() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div class="flex items-center">
                    <div class="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center">
                        <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.996-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path></svg>
                    </div>
                    <div class="ml-3">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="border-b border-gray-200">
                <nav class="flex space-x-8 px-6" aria-label="Tabs">
                    <button onclick="filterByStatus('all')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-blue-500 text-blue-600" data-status="all">Semua</button>
                    <button onclick="filterByStatus('OPEN')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-status="OPEN">Menunggu</button>
                    <button onclick="filterByStatus('IN_PROGRESS')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-status="IN_PROGRESS">Diproses</button>
                    <button onclick="filterByStatus('CLOSED')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-status="CLOSED">Selesai</button>
                    @if(auth()->user()?->pengurus?->role && in_array(auth()->user()->pengurus->role->NAME, ['ADM', 'ADMIN_DPP', 'ADMIN_DPW', 'ADMIN_DPD']))
                    <button onclick="filterByJenis('ADVOKASI')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-jenis="ADVOKASI">Advokasi</button>
                    <button onclick="filterByJenis('ASPIRASI')" class="filter-tab border-b-2 py-4 px-1 text-sm font-medium border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-jenis="ASPIRASI">Aspirasi</button>
                    @endif
                </nav>
            </div>
            <div class="p-4">
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" placeholder="Cari berdasarkan judul, deskripsi, atau nama anggota..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" onkeyup="searchKonsultasi()">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div id="konsultasiList">
                ...
                <div class="konsultasi-item border-b border-gray-200 last:border-b-0 p-6 hover:bg-gray-50 transition" 
                     ...
                     data-nama="{{ strtolower($item->karyawan?->V_NAMA_KARYAWAN ?? '') }}">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex-1">
                            <div class="flex items-center space-x-3 mb-2">
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $item->JENIS === 'ADVOKASI' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700' }}">{{ $item->JENIS }}</span>
                                <span class="px-2 py-1 text-xs font-medium rounded-full bg-{{ $item->status_color ?? 'gray' }}-100 text-{{ $item->status_color ?? 'gray' }}-700">{{ $item->status_text ?? 'Unknown' }}</span>
                                <span class="text-xs text-gray-500">{{ $item->TUJUAN }} {{ $item->TUJUAN_SPESIFIK ? '- ' . $item->TUJUAN_SPESIFIK : '' }}</span>
                                @if($item->KATEGORI_ADVOKASI)
                                    <span class="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-full">{{ $item->KATEGORI_ADVOKASI }}</span>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center text-xs text-gray-500 space-x-4">
                                ...
                                @if($item->updated_at && $item->updated_at != $item->created_at)
                                    <span class="flex items-center">
                                        ...
                                        Update: {{ $item->updated_at ? \Carbon\Carbon::parse($item->updated_at)->format('d M Y') : '-' }}
                                    </span>
                                @endif
                            </div>
                        </div>
                        <div class="ml-4 flex flex-col space-y-2">
                            ...
                            @if($item->created_at && \Carbon\Carbon::parse($item->created_at)->diffInDays(now()) > 7 && $item->STATUS !== 'CLOSED')
                                <span class="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-full font-medium">
                                    📅 {{ \Carbon\Carbon::parse($item->created_at)->diffInDays(now()) }} hari
                                </span>
                            @endif
                        </div>
                    </div>
            ...
            <div id="noResults" class="p-12 text-center hidden">
                <svg class="w-16 h-16 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
