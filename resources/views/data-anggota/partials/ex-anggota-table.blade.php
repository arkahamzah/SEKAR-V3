<table class="w-full">
    <thead class="bg-gray-50">
        <tr>
            ...
            <th class="text-left py-3 px-4 font-semibold text-gray-700 text-xs">POSISI TERAKHIR</th>
            <th class="text-left py-3 px-4 font-semibold text-gray-700 text-xs">TANGGAL KELUAR</th>
            <th class="text-left py-3 px-4 font-semibold text-gray-700 text-xs">ALASAN KELUAR</th>
...
@if(isset($ex_anggota) && $ex_anggota->hasPages())
<div class="px-6 py-4 border-t border-gray-200">
    <div class="flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Menampilkan {{ $ex_anggota->firstItem() }} sampai {{ $ex_anggota->lastItem() }} dari {{ $ex_anggota->total() }} data
        </div>
        <div class="flex space-x-1">
            {{ $ex_anggota->appends(request()->query())->links('pagination::tailwind') }}
        </div>
    </div>
</div>
@endif